...
public:
    ...
    void Dijkstra(const int n/*節點總數*/, const int v/*起點*/);
    void BellmanFord(const int n, const int v);
    void FloydWarshall(const int n);
    void showGraph();
};

class adjListWeightUndirectedGraph : public adjListWeightDirectedGraph{//鄰接串列表式的有權重無向圖
public:
    adjListWeightUndirectedGraph(int v = 1);
    void InsertEdge(int u,int v,int w = 1);
    void InsertEdge(pair<int,pair<int,int>> edge);
    void DeleteEdge(int u,int v);
    void showGraph();
