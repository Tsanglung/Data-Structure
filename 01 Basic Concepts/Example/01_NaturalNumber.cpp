...
class NaturalNumber{
    ...
    public:
        NaturalNumber(unsigned int v = 0) {
            if(v <= MAXINT) value = v;
            else throw out_of_range("value exceeds MAXINT.");
        }
        ...
        static NaturalNumber Add(const NaturalNumber& x, const NaturalNumber& y){
            if(x.value < MAXINT - y.value) return NaturalNumber(x.value + y.value);
            else return NaturalNumber(MAXINT);
        }
        ...
        static NaturalNumber Successor(const NaturalNumber& x){
            if(x.value == MAXINT) return NaturalNumber(x.value);
            else return NaturalNumber(x.value + 1);
        }
        static NaturalNumber Subtract(const NaturalNumber& x,const NaturalNumber& y){
            if(x.value < y.value) return NaturalNumber(0);
            else return NaturalNumber(x.value - y.value);
        }
